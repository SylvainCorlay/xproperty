...
namespace xp
{
    ...
    public:
        ...
        template<size_t N>
        xproperty(owner_type* owner, char const (&name)[N]) XP_NOEXCEPT(value_type);
        template <class V, size_t N>
        xproperty(owner_type* owner, char const (&name)[N], V&& value) XP_NOEXCEPT(value_type);
        template <class V, class LV, size_t N>
        xproperty(owner_type* owner, char const (&name)[N], V&& value, LV&& lambda_validator) XP_NOEXCEPT(value_type);
    ...
    inline xproperty<T, O>::xproperty(owner_type* owner,
                                      char const (&name)[N]) XP_NOEXCEPT(value_type)
